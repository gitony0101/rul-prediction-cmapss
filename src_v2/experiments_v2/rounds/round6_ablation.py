...
def resolve_output_root(output_root: str) -> Path:
    default_root = (_PROJECT_ROOT / "outputs_v2" / "round6_ablation").resolve()
    p = Path(output_root)
    ...
    try:
        ...
        warnings.warn(
            f"Resolved output_root escaped project root: {resolved}. Falling back to project-local outputs_v2/round6_ablation.",
            RuntimeWarning,
        )
        return default_root

    allowed_root = (_PROJECT_ROOT / "outputs_v2").resolve()
    try:
        resolved.relative_to(allowed_root)
    except ValueError:
        warnings.warn(
            f"Round 6 output_root must stay under project-local outputs_v2. Got: {resolved}. Falling back to {default_root}.",
            RuntimeWarning,
        )
        return default_root
...
def main() -> None:
    ...
    print(f"[INFO] Round 6 output root resolved to: {output_root}")
    output_root.mkdir(parents=True, exist_ok=True)

    allowed_outputs_root = (_PROJECT_ROOT / "outputs_v2").resolve()
    try:
        output_root.relative_to(allowed_outputs_root)
    except ValueError:
        raise RuntimeError(
            f"Round 6 output root escaped project-local outputs_v2: {output_root}"
        )
