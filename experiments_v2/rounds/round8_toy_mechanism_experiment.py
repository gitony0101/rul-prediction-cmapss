...
@dataclass
class TrainConfig:
    output_root: str = "outputs_v2/round8_toy_mechanism"
...
def run_train_for_group_scenario_seed(
    ...
    pd.DataFrame(history).to_csv(group_dir / "train_history.csv", index=False)
    pred_df.to_csv(group_dir / "test_predictions.csv", index=False)
    save_json(
        {
            "group": group,
            "scenario": scenario,
    ...
    torch.save(best_state, group_dir / "best_model.pt")
    LOGGER.info("Saved outputs to %s", group_dir)
...
def run_maintenance_for_group_scenario_seed(
    ...
    save_json(result, group_dir / "maintenance_metrics.json")
    pd.DataFrame(result["round_logs"]).to_csv(
        group_dir / "maintenance_round_logs.csv", index=False
    )
    LOGGER.info("Maintenance saved to %s", group_dir)
...
def summarize_round8(output_root: Path) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    numeric_cols = [c for c in detail_df.columns if c not in set(group_cols + ["seed"])]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output_root", default="outputs_v2/round8_toy_mechanism")
...
def main() -> None:
    ...
    if args.action == "summary":
        ...
        detail_df.to_csv(output_root / "round8_detail.csv", index=False)
        summary_df.to_csv(output_root / "round8_summary.csv", index=False)
        LOGGER.info(
            "Saved %s and %s",
            output_root / "round8_detail.csv",
