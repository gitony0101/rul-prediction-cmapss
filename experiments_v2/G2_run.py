...
from src_v2.rul.training.loss import MSELoss
...
@dataclass
class Config:
    experiment_name: str = "G2_CNN_BiLSTM_MSE_MCD_v2"
    dataset_name: str = "FD001"
    seed: int = 42

    data_dir: str = "CMAPSSData"
    output_root: str = "outputs_v2/G2_CNN_BiLSTM_MSE_MCD"
    ...
    mc_samples_val: int = 30
    mc_samples_test: int = 100
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run G2 experiment")
...
def apply_overrides(cfg: Config, args: argparse.Namespace) -> Config:
    ...
    if args.mc_samples_val is not None:
        cfg.mc_samples_val = args.mc_samples_val
    if args.mc_samples_test is not None:
...
def main():
    ...
    for epoch in range(1, cfg.num_epochs + 1):
        ...
        val_true_det, val_pred_det, _ = trainer.predict(val_loader)

        val_rmse_det = rmse(val_true_det, val_pred_det)
        val_mae_det = mae(val_true_det, val_pred_det)
        val_nasa_det = nasa_score(val_true_det, val_pred_det)

        row = {
            "epoch": epoch,
            "train_loss": train_loss,
            "val_loss": val_loss,
            "val_rmse_det": val_rmse_det,
            "val_mae_det": val_mae_det,
            "val_nasa_det": val_nasa_det,
        }
        history.append(row)
        ...
        print(
            ...
            f"val_rmse_det={val_rmse_det:.4f} "
            f"val_mae_det={val_mae_det:.4f} "
            f"val_nasa_det={val_nasa_det:.4f}"
        )

        if val_rmse_det < best_val_rmse:
            best_val_rmse = val_rmse_det
            best_epoch = epoch
    ...
    val_true_det, val_pred_det, val_meta = trainer.predict(val_loader)
    test_true_det, test_pred_det, test_meta = trainer.predict(test_loader)

    val_true_mcd, val_pred_mcd, val_sigma, val_meta_mcd = mc_dropout_predict(
        model=model,
        loader=val_loader,
        device=device,
        n_samples=cfg.mc_samples_val,
    )
    test_true_mcd, test_pred_mcd, test_sigma, test_meta_mcd = mc_dropout_predict(
        model=model,
        loader=test_loader,
        device=device,
        n_samples=cfg.mc_samples_test,
    )

    val_metrics = {
        "rmse": to_serializable_float(rmse(val_true_mcd, val_pred_mcd)),
        "mae": to_serializable_float(mae(val_true_mcd, val_pred_mcd)),
        "nasa": to_serializable_float(nasa_score(val_true_mcd, val_pred_mcd)),
        "pred_std_mean": to_serializable_float(pd.Series(val_sigma).mean()),
        "pred_std_std": to_serializable_float(pd.Series(val_sigma).std(ddof=0)),
        "mc_samples": int(cfg.mc_samples_val),
    }

    test_metrics = {
        "rmse": to_serializable_float(rmse(test_true_mcd, test_pred_mcd)),
        "mae": to_serializable_float(mae(test_true_mcd, test_pred_mcd)),
        "nasa": to_serializable_float(nasa_score(test_true_mcd, test_pred_mcd)),
        "pred_std_mean": to_serializable_float(pd.Series(test_sigma).mean()),
        "pred_std_std": to_serializable_float(pd.Series(test_sigma).std(ddof=0)),
        "mc_samples": int(cfg.mc_samples_test),
    }

    # Save test predictions with uncertainty
    pred_df = pd.DataFrame(test_meta_mcd)
    pred_df["y_true"] = pd.Series(test_true_mcd).astype(float)
    pred_df["y_pred"] = pd.Series(test_pred_mcd).astype(float)
    pred_df["pred_mean"] = pd.Series(test_pred_mcd).astype(float)
    pred_df["pred_std"] = pd.Series(test_sigma).astype(float)
    ...
    print(f"Outputs saved to: outputs_v2/G2_CNN_BiLSTM_MSE_MCD/{Path(run_dir).name}")
